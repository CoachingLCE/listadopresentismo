 ...
 * que las manda de verdad (api/cron/alertas) como por la pantalla "Emails" que las
 * previsualiza aunque el envío automático todavía no esté configurado.
 */
export function construirDigestAlertas(alertas) {
  const asunto = `⚠ Presentismo ILCE — ${alertas.length} alerta(s) nueva(s)`;
  const html = `
    <div style="font-family:sans-serif;color:#111">
      <h2 style="margin-bottom:4px">⚠ Alertas de Presentismo ILCE</h2>
      <p style="color:#555;font-size:13px">Se detectaron ${alertas.length} situación(es) nueva(s) en Reportes:</p>
      <ul style="padding-left:18px">
        ${alertas.map((a) => `<li style="margin-bottom:10px"><strong>${a.texto}</strong><br/><span style="color:#777;font-size:12px">${a.motivo}</span></li>`).join('')}
      </ul>
