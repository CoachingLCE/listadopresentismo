...
// SuperAdmin, Coordinación y Académico — nunca a Docente/Staff.
export function puedeVerComoOtro(usuario) {
  return tieneAlguno(usuario, ['SuperAdmin', 'Coordinacion', 'Academico']);
}

// Ver/cargar Seguimiento de un estudiante — gestión académica de siempre, más el
// Docente (pero el Docente solo ve/carga seguimiento de sus propios estudiantes,
// filtrado por las ediciones que tiene asignadas — ver el filtro en /api/seguimiento).
export function tienePermisoVerSeguimiento(usuario) {
  return tieneAlguno(usuario, ['SuperAdmin', 'Coordinacion', 'Academico', 'Docente']);
}
