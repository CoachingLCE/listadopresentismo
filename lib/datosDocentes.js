...
// Pestaña "Docentes" — columnas: Id, Nombre, Email, Cursos, Activo, Roles (opcional).
// "Roles" es nueva: si esa columna todavía no existe en el Sheet (o está vacía en una
// fila), se asume ['Docente'] — así el roster existente sigue funcionando igual hasta que
// se agregue la columna "Roles" en la fila 1 de la pestaña "Docentes".
function parseRoles(valor) {
  const roles = (valor || '').split(',').map((r) => r.trim()).filter(Boolean);
...
export async function leerDocentesCombinados() {
  ...
  const fijos = DOCENTES_DEFAULT
    ...
    .map((d) => ({ id: `fijo-${d.email}`, nombre: d.nombre, email: d.email.toLowerCase(), cursos: d.cursos, roles: d.roles || ['Docente'], activo: true, _rowIndex: null }));
  return [...fijos, ...delSheet].filter((d) => d.activo);
}

export async function agregarDocente({ nombre, email, cursos, roles }) {
  await appendRow('Docentes', {
    Id: `doc-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    Nombre: nombre,
    Email: (email || '').trim().toLowerCase(),
    Cursos: (cursos || []).join(', '),
    Roles: (roles && roles.length > 0 ? roles : ['Docente']).join(', '),
    Activo: 'TRUE'
  });
}
...
export async function actualizarDocente(rowIndex, cambios) {
  ...
  if (cambios.roles !== undefined) patch.Roles = (cambios.roles.length > 0 ? cambios.roles : ['Docente']).join(', ');
...
export async function materializarDocente(docente, cambios = {}) {
  ...
  const roles = cambios.roles !== undefined ? cambios.roles : (docente.roles || ['Docente']);
  ...
  await appendRow('Docentes', {
    ...
    Email: (email || '').trim().toLowerCase(),
    Cursos: (cursos || []).join(', '),
    Roles: (roles.length > 0 ? roles : ['Docente']).join(', '),
