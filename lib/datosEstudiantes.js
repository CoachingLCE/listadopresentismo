 ...
 * Devuelve los estudiantes creados (id, nombre, fechaIncorporacion) para que quien
 * llama pueda, si corresponde, marcarles automáticamente las clases anteriores a su
 * incorporación (ver POST /api/estudiantes). */
export async function agregarEstudiantesBulk(edicionId, nombres) {
  ...
  await appendRows('Estudiantes', nuevos);
  return nuevos.map((f) => ({ id: f.Id, nombre: f.Nombre, fechaIncorporacion: f.FechaIncorporacion }));
}
